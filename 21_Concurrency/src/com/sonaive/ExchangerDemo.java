package com.sonaive;//: concurrency/ExchangerDemo.java
  ...
  public void run() {
    try {
      while(!Thread.interrupted()) {
        ...
        System.out.println("Producer holder size " + holder.size());
      }
...
class ExchangerConsumer<T> implements Runnable {
  ...
  public void run() {
    try {
      while(!Thread.interrupted()) {
        holder = exchanger.exchange(holder);
        System.out.println("Consumer holder size " + holder.size());
        for(T x : holder) {
